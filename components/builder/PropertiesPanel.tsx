...
function loadCustomGoogleFont(input: string) {
  if (typeof document === "undefined") return;
  const name = input.replace(/['"]/g, "").split(",")[0].trim();
  if (!name || loadedCustomFonts.has(name)) return;
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="text-[9px] uppercase tracking-widest text-white/30 block mb-1.5 font-semibold">
      {children}
    </label>
  );
}
...
function Input({
  ...
  return (
    <input
      ...
      className="w-full text-[11px] bg-[#1c1c1c] border border-white/8 rounded-md px-2.5 py-1.5 outline-none focus:border-blue-500/60 focus:bg-[#202020] text-white/80 placeholder-white/15 transition-colors font-medium cursor-text [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
    />
  );
}
  ...
  return (
    <div className="relative">
      <button
        ...
        type="button"
        onClick={toggle}
        className="w-full flex items-center justify-between text-[11px] bg-[#1c1c1c] border border-white/8 rounded-md px-2.5 py-1.5 text-white/80 font-medium hover:border-white/15 hover:bg-[#202020] transition-colors cursor-pointer"
      >
  ...
  return (
    <div>
      <button
        ...
        className={`w-full flex items-center justify-between py-2.5 group ${collapsible ? "cursor-pointer" : "cursor-default"}`}
      >
        <span
          className={`text-[9px] uppercase tracking-[0.12em] font-bold transition-colors duration-150 ${
            collapsible
              ? open
...
export default function PropertiesPanel() {
  ...
  if (!el)
    return (
      <div className="w-[220px] border-l border-[#222] bg-[#141414] flex flex-col items-center justify-center p-6 text-center h-full shrink-0">
        <div className="w-8 h-8 rounded-lg bg-white/4 flex items-center justify-center mb-3">
          <MousePointer2 className="w-4 h-4 text-white/15" />
        </div>
  ...
  const toggleStyle = (
    ...
    updateElement(el.id, {
      styles: {
        ...el.styles,
        [key]: el.styles[key] === onVal ? offVal : onVal,
      },
    });
  };
  ...
  const updateAllBorderSides = (value: string) => {
    updateElement(el.id, {
      styles: {
        ...el.styles,
        borderTop: value,
  ...
  return (
    <div className="w-[220px] border-l border-[#222] bg-[#141414] flex flex-col shrink-0 overflow-hidden select-none h-full">
      <div className="h-10 px-3 border-b border-[#222] flex items-center justify-between bg-[#161616] shrink-0">
        <div className="flex items-center gap-2">
          <Layers className="w-3 h-3 text-blue-400/80" />
          <span className="text-[11px] font-bold text-white/70 capitalize tracking-wide">
            {el.type}
          </span>
        </div>
        <span className="text-[9px] text-white/15 font-mono bg-white/5 px-1.5 py-0.5 rounded">
          {el.id.slice(-4)}
        </span>
      </div>
      ...
      <div ref={panelRef} className="flex-1 overflow-y-auto px-3">
        <Section title="Content & Text">
          <textarea
            ...
            className="w-full text-[11px] bg-[#1c1c1c] border border-white/8 rounded-md px-2.5 py-2 outline-none focus:border-blue-500/60 text-white/80 resize-none font-medium h-14 placeholder-white/15 cursor-text"
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(v) => {
                  ...
                  if (v.trim()) loadCustomGoogleFont(v.trim());
                }}
        ...
        <Section
          ...
          collapsible
          defaultOpen={
            el.styles.display === "flex" || el.styles.display === "grid"
          }
        >
          <Row>
            <Field label="Display">
          ...
          {el.styles.display === "flex" && (
            <div className="space-y-2.5 bg-white/3 rounded-lg p-2.5 border border-white/5">
              ...
              <Field label="Justify">
                <div className="flex gap-1 bg-black/20 p-1 rounded-md">
                  {[
                    {
                      id: "flex-start",
                      icon: <AlignVerticalJustifyStart size={11} />,
                    },
                    {
                      id: "center",
                      icon: <AlignVerticalJustifyCenter size={11} />,
                    },
                    {
                      id: "flex-end",
                      icon: <AlignVerticalJustifyEnd size={11} />,
                    },
                  ].map((opt) => (
                    <button
                      key={opt.id}
                      onClick={() => updateStyle("justifyContent", opt.id)}
                      className={`flex-1 py-1.5 rounded flex items-center justify-center transition-all cursor-pointer ${el.styles.justifyContent === opt.id ? "bg-[#333] text-white" : "text-white/25 hover:text-white/50"}`}
                    >
                      {opt.icon}
                    </button>
                  ))}
              ...
              <Field label="Align">
                <div className="flex gap-1 bg-black/20 p-1 rounded-md">
                  {[
                    { id: "flex-start", icon: <AlignLeft size={11} /> },
                    { id: "center", icon: <AlignCenter size={11} /> },
                    { id: "flex-end", icon: <AlignRight size={11} /> },
                    { id: "stretch", icon: <StretchHorizontal size={11} /> },
                  ].map((opt) => (
                    <button
                      key={opt.id}
                      onClick={() => updateStyle("alignItems", opt.id)}
                      className={`flex-1 py-1.5 rounded flex items-center justify-center transition-all cursor-pointer ${el.styles.alignItems === opt.id ? "bg-[#333] text-white" : "text-white/25 hover:text-white/50"}`}
                    >
                      {opt.icon}
                    </button>
                  ))}
                  ...
                  <div className="grid grid-cols-4 gap-1 mt-2">
                    {[
                      ...
                      <button
                        ...
                        className={`py-1 text-[11px] rounded transition-all border cursor-pointer ${
                          (el.styles.gradientAngle ?? 135) === a
                            ? "bg-blue-500/20 border-blue-500/40 text-white"
                            : "bg-white/3 border-white/5 text-white/30 hover:text-white/70 hover:bg-white/8"
                        }`}
                      >
                        {l}
                      </button>
